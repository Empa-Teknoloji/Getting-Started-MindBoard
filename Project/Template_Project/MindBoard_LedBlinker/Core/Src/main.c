...
GPIO_TypeDef* LED_PORTS[NUM_LEDS] = {
    GPIOA, GPIOB, GPIOB, GPIOA, GPIOB, GPIOB, GPIOB, GPIOB, GPIOC, GPIOC
};
uint16_t LED_PINS[NUM_LEDS] = {
    GPIO_PIN_1, GPIO_PIN_1, GPIO_PIN_2, GPIO_PIN_0, GPIO_PIN_12, GPIO_PIN_13, GPIO_PIN_14, GPIO_PIN_15, GPIO_PIN_13, GPIO_PIN_14
};
...
void LED_Dance(void)
{
    ...
    for (int i = 0; i < NUM_LEDS; i++) {
        HAL_GPIO_WritePin(LED_PORTS[i], LED_PINS[i], GPIO_PIN_SET); // Turn on LED
        HAL_Delay(100); // Delay 100ms
        HAL_GPIO_WritePin(LED_PORTS[i], LED_PINS[i], GPIO_PIN_RESET); // Turn off LED
    }
    ...
    for (int j = 0; j < 3; j++) { // Blink 3 times
        for (int i = 0; i < NUM_LEDS; i++) {
            HAL_GPIO_WritePin(LED_PORTS[i], LED_PINS[i], GPIO_PIN_SET); // Turn on all LEDs
        }
        HAL_Delay(300); // Delay 300ms

        for (int i = 0; i < NUM_LEDS; i++) {
            HAL_GPIO_WritePin(LED_PORTS[i], LED_PINS[i], GPIO_PIN_RESET); // Turn off all LEDs
        }
        HAL_Delay(300); // Delay 300ms
    }
}
